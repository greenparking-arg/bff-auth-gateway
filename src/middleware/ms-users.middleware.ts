...
@Injectable()
export class MsUsersMiddleware implements NestMiddleware {
  private readonly logger = new Logger(MsUsersMiddleware.name);
  ...
  use(req: Request, res: Response, next: NextFunction) {
    const target = this.configService.get<string>('URL_MS_USERS');
    ...
    const proxy = createProxyMiddleware({
      ...
      on: {
        proxyReq: (proxyReq, req: Request, res: Response) => {
          this.logger.debug(`Proxying MS-USERS - ${req.method} request to: ${target}${req.url}`);
